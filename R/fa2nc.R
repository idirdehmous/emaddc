...
# R SCRIPT TO CONVERT AROME1.3 OR ALARO1.3 KM PRECIPITATION 
# FIELD DIRECTLY TO netCDF  
...
files   =paste0(basedir,"/",period,"/",exp,"/",year ,lz(month),lz(day),lz(starth),"/","PF",model,"be13b_l+",lz(seq(1,6,1),4))
...
# GET THE PCP FIELDS 
pcp_all=lapply(files, function(fa) FAdec(fa,"SURFACCPLUIE") )
...
# NETCDF OUTFILE 
ncfname=paste("ar13_",year ,lz(month),lz(day),lz(starth),"_1h.nc", sep="")

# META DATA 
# ( IN pysteps IT WORKED WITH THIS COORDINATE )
...
# x, y  ATTRIBUTES 
ncatt_put(ncout ,"x","standard_name","projection_x_coordinate") ; ncatt_put(ncout ,"y","standard_name","projection_y_coordinate")
